...
const ALL_PROJECTS: GuidedProject[] = [
	{
		...
		description: "Build a web app that extracts skills from resumes and suggests roles using embeddings.",
	...
	},
	{
		...
		tags: ["react", "typescript", "tailwind", "auth", "crud", "node", "express", "mongo"],
		difficulty: "Beginner",
		guideUrl: "https://example.com/fullstack-task-manager"
	},
	{
		title: "Realtime Chat with WebSockets",
		description: "Channel-based chat with presence, typing indicators, and message persistence.",
	...
	},
	{
		...
		description: "Product catalog, cart, checkout, and order tracking with modern UI.",
...
const KNOWN_SKILLS = [
	"react", "typescript", "javascript", "tailwind", "css", "html", "redux", "next",
	"node", "express", "mongo", "postgres", "sql", "redis",
	"nlp", "embeddings", "ml", "python",
	"websockets", "graphql", "stripe",
	"testing", "jest", "vitest",
	"d3", "charts", "etl", "pwa", "search", "indexing", "auth", "rest"
];
...
function scoreProjectForSkills(project: GuidedProject, skills: string[]): number {
	const base = project.tags.reduce((acc, tag) => acc + (skills.includes(tag) ? 2 : 0), 0);
	
	if (skills.includes("react") && skills.includes("typescript") && project.tags.includes("react")) {
		return base + 1;
	}
	return base;
}
...
function rankProjects(skills: string[]): GuidedProject[] {
	...
	return withScores.filter((x) => x.score > 0).slice(0, 5).map((x) => x.project);
}
...
const ProjectSuggestions = () => {
	...
	async function analyze(textOverride?: string) {
		...
		setIsLoading(true);
		try {
			const sourceText = (typeof textOverride === "string" && textOverride.length > 0) ? textOverride : inputText;
			const extracted = extractSkillsFromText(sourceText);
			setSkills(extracted);
			try {
				const apiBase = (import.meta as any).env?.VITE_API_BASE || "http://localhost:5175";
				...
				if (resp.ok) {
					...
					if (Array.isArray(data) && data.length > 0) {
						const normalized = data.slice(0, 5).map((p: any) => ({
							...
							difficulty: (p.difficulty === "Beginner" || p.difficulty === "Intermediate" || p.difficulty === "Advanced") ? p.difficulty : "Intermediate",
	...
	return (
		<section id="suggestions" className="border-t border-slate-200">
			<div className="mx-auto max-w-7xl px-4 py-14 sm:px-6">
				<header className="mx-auto max-w-3xl text-center">
					<h2 className="section-title">Get Tailored Project Suggestions</h2>
					<p className="section-subtitle">Upload your PDF resume (or paste text). We'll suggest 5 trending guided projects matched to your skills.</p>
				</header>
				...
				<div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
					<div className="lg:col-span-1">
						<div className="card">
							...
							<div className="mt-4">
								<label className="block text-sm font-medium text-slate-700">Or paste your resume</label>
								<textarea
									value={inputText}
							...
							<div className="mt-4 flex items-center gap-3">
								<button onClick={() => analyze()} disabled={!inputText || isLoading} className="btn-gradient disabled:opacity-50">
									{isLoading ? "Analyzing..." : "Analyze"}
								</button>
								<button onClick={reset} className="rounded-md border border-slate-300 px-4 py-2 text-sm">Clear</button>
							</div>
					...
					<div className="lg:col-span-2">
						<div className="card">
							<h3 className="text-lg font-semibold text-slate-900">Suggested Projects</h3>
							{skills.length > 0 && (
								<p className="mt-1 text-sm text-slate-600">Matched skills: {skills.join(", ")}</p>
							)}
							<div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
								{suggestions.length === 0 && !isLoading ? (
									<p className="text-sm text-slate-600">No suggestions yet. Paste your resume and click Analyze.</p>
								) : (
									<>
										{suggestions.map((p) => (
											<div key={p.title} className="rounded-lg border border-slate-200 p-4">
												<div className="flex items-start justify-between gap-3">
													...
													<span className="rounded-full border border-slate-300 px-2 py-1 text-xs text-slate-700">{p.difficulty}</span>
												</div>
												...
												<div className="flex gap-3">
													{p.guideUrl && (
														<div className="mt-3">
															<a href={p.guideUrl} target="_blank" rel="noreferrer" className="btn-gradient inline-flex items-center gap-2 text-sm flex-1">
													...
													<button
														...
														disabled={downloadingProject === p.title || isLoading}
														className="btn-gradient inline-flex items-center gap-2 text-sm mt-2 flex-1"
													>
														{downloadingProject === p.title ? "Downloading..." : "Download Project PDF"}
													</button>
												</div>
											</div>
										))}
