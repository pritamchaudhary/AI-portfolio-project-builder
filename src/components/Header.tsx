		...
		<header className="sticky top-0 z-50 w-full border-b border-slate-200 bg-white/80 backdrop-blur">
			<div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6">
				<a href="#hero" className="flex items-center gap-2" aria-label="AI Portfolio Project Builder - Home">
					...
					<span className="text-sm font-semibold tracking-tight sm:text-base">AI Portfolio Project Builder</span>
				</a>

				<nav className="hidden items-center gap-6 md:flex" aria-label="Primary Navigation">
					{navItems.map((item) => (
						<a key={item.href} href={item.href} className="text-sm text-slate-600 transition-colors hover:text-slate-900" role="link">
							{item.label}
						</a>
					))}
			...
			{open && (
				<div className="border-t border-slate-200 md:hidden">
					<nav className="mx-auto flex max-w-7xl flex-col px-4 py-3 sm:px-6" aria-label="Mobile Navigation">
						{navItems.map((item) => (
							<a key={item.href} href={item.href} className="rounded-md px-3 py-2 text-sm text-slate-700 hover:bg-slate-100" onClick={() => setOpen(false)}>
								{item.label}
							</a>
						))}
